...
# 1. If WordPress core is missing, copy it from the image source.
...
# 3. Safety repair: keep SQLite integration inside persistent wp-content.
...
# 4. Replace WordPress wp-content with persistent wp-content.
...
# If not, copy the current image/runtime config once.
if [ -f "$PERSISTENT_WP_CONFIG" ]; then
  echo "/data/wp-config.php exists. Using it."
else
  echo "/data/wp-config.php does not exist. Creating it from existing config..."

  if [ -f "$WORDPRESS_TARGET_DIR/wp-config.php" ] && [ ! -L "$WORDPRESS_TARGET_DIR/wp-config.php" ]; then
    echo "Copying /var/www/html/wp-config.php -> /data/wp-config.php"
    php -l "$WORDPRESS_TARGET_DIR/wp-config.php"
    cp -a "$WORDPRESS_TARGET_DIR/wp-config.php" "$PERSISTENT_WP_CONFIG"

  elif [ -f "$WORDPRESS_TARGET_DIR/wp-config-docker.php" ]; then
    echo "Copying /var/www/html/wp-config-docker.php -> /data/wp-config.php"
    php -l "$WORDPRESS_TARGET_DIR/wp-config-docker.php"
    cp -a "$WORDPRESS_TARGET_DIR/wp-config-docker.php" "$PERSISTENT_WP_CONFIG"

  elif [ -f "$WORDPRESS_SOURCE_DIR/wp-config-docker.php" ]; then
    echo "Copying /usr/src/wordpress/wp-config-docker.php -> /data/wp-config.php"
    php -l "$WORDPRESS_SOURCE_DIR/wp-config-docker.php"
    cp -a "$WORDPRESS_SOURCE_DIR/wp-config-docker.php" "$PERSISTENT_WP_CONFIG"

  else
    echo "ERROR: no wp-config.php or wp-config-docker.php source found."
    exit 1
  fi
fi

# 6. Validate persistent config before linking.
php -l "$PERSISTENT_WP_CONFIG"

# 7. Replace live wp-config.php with persistent config symlink.
